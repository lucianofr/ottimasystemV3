...
### B-F4-05 — Minors m4: vírgula/ponto, "Aplicar" fecha, booleano com fonte booleana, EU nas portas com fonte, grade sem sobreposição
...
| 5 | Observar o valor ao vivo de uma porta que carrega um **booleano de verdade**, com o flow rodando: `Leitura OPC` de tag `data_type: bool` (ex.: `ns=2;s=sim.bool.square` do opcsim). `tab.screenshot()` do valor exibido na porta no canvas. | Valor booleano exibido como **booleano** em pt-BR (`verdadeiro`/`falso`) — **nunca** "1"/"0", "1.0"/"0.0" nem outro número cru: exibir número era exatamente o bug m4; Regra do Canal Redundante. **Emenda de 2026-08-05 (execução do gate):** o enunciado original mandava usar uma "saída booleana de Script", inalcançável por arquitetura — a saída de Script é coagida a `float` no pool (`script_pool.py`: `outputs[port] = float(scope[port])`, spec §3.3) e a entrada também (`blocks/script.py`: decisão A-5, "o script só vê números; `True` vira 1.0"). Logo `OUT1 = True` chega ao barramento como `1.0` e é exibido como `1` corretamente. A única fonte de booleano do sistema hoje é a tag OPC `bool` — é nela que o item m4 do booleano se prova. Mesma classe da emenda de EU (Script/TFS sem fonte), sem mudança de spec. |
