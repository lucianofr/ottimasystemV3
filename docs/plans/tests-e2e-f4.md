...
### B-F4-05 — Minors m4: vírgula/ponto, "Aplicar" fecha, booleano de Script, EU nas portas, grade sem sobreposição

**Objetivo:** confirmar o fechamento dos 5 itens do débito m4 (spec F4 §8) nas superfícies afetadas.
**Rastreabilidade:** spec F4 §8 tabela, linha `m4` · plano F4a tarefa 0.7 (m1+m4 exceto EU nas portas) e tarefa 4.1 (EU nas portas, mesma leva das portas dinâmicas do MPC).
...
| 6 | `tab.screenshot()` de um nó TFS e de um nó Script no canvas, focando as portas. | Toda porta de Script e de TFS exibe a **EU** ao lado do valor (Regra do Número Tabular — "número sem unidade de engenharia é defeito"), mesmo que a EU seja uma string vazia/genérica configurada pelo usuário. |
