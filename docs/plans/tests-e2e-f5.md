...
| 6 | Ligar a pena de SP da CV pelo checkbox da linha `[data-var-id="sp:<cv_id>"]` (o SP é opt-in desde a emenda 2026-08-16 — não desenha sozinho). `tab.evaluate` no `fetch` do passo 2 localizando, no array `auto`, o trecho anterior à confirmação do AUTO (B-F5-03, passo 6) — deve conter `false`. `tab.screenshot()` desse trecho da pena de SP. | Pena de SP pontilhada no matiz da própria CV (nunca no Azul Único), dessaturada no trecho `auto=false` (SP em PV-tracking não é SP comandado — §2.2-1, F5R-21); saturada no trecho `auto=true` seguinte. |
...
| 8 | `tab.screenshot()` da legenda do trend (`[data-testid="operate-trend-legend"]`, provável). | CVs (só PV) ligadas por default até o teto de 8 penas, na ordem do config; abaixo de cada CV a linha `SP · <nome>` com faixa pontilhada da mesma cor, **desligada**; MVs e DVs desligadas por default (opt-in, decisão A-11 emendada em 2026-08-16). |
