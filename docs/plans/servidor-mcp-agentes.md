...
3. Ferramentas de escrita (subscribe → POST → aguardar → devolver resultado). Predicado por ferramenta, **nenhum usa `vars[mv_id].v` como sucesso** (é física, não comando — ADR-028). **`events` é canal GLOBAL sem escopo por flow/bloco no protocolo — todo ramo de evento DEVE checar `payload["origin"] == f"flow:{flow_id}/block:{block_id}"` (mesmo formato de `mpc.py:169`/`events.py:85`), senão o evento de OUTRO bloco com o mesmo `kind`/`var_id` confirma falsamente** (achado de revisão — o ramo de `mpc_state` não precisa, o subscribe já o escopa por `{flow_id}/{block_id}` no servidor):
