...
- Execução da fase em **git worktree próprio** (CLAUDE.md §Workflow): `../ottimaSystemV3-f1`, branch `feat/f1-fundacao`, base `fa7711f`.
...
- **Decisão de pré-flight (usuário, 2026-08-03):** os 3 `main.py` de worker (Task 12) permanecem **arquivos separados e quase idênticos** por escolha explícita — serviços são deployáveis independentes que divergem em F2/F3 (asyncua, scan cycle, gravação). Fatorar um `create_worker_app` compartilhado seria abstração prematura. **Não é achado de revisão.**
