...
**Versão do documento:** 2.6 · 2026-08-17 · **Status:** aprovado para implementação (F1-F6 concluídas)
...
**Changelog 2.6:** `sp_range_pct` (RF-615) passa a valer também para CV **integradora** — reaproveita o mesmo campo/UX da CV `selfreg` (nenhum campo novo): em vez de banda de nível, vira o **ε de taxa da linha** (`pct/100 × span/tss`), no lugar do `economics.integrating_tolerance` do bloco só para essa CV (sem o campo, cai no `integrating_tolerance` do bloco — comportamento anterior bit a bit). Fecha a lacuna do ADR-027 §4: antes, a única forma de uma CV integradora ceder espaço pros outros objetivos do SSTO era `priority` ou o `integrating_tolerance` global do bloco. Restrição integradora **não** ganhou o campo (não tem `sp_range_pct` — mudaria schema); `ε` continua por-ciclo, não cumulativo. ADR-027 revisado (emenda). PRD avança de 2.5 para v2.6 (2026-08-17).
**Autor:** Luciano França Rocha (LFR Automação), consolidado em sessão de grilling
**Documentos-irmãos (normativos):** `adr/ADR-001 … ADR-034` · `GLOSSARY.md`
...
O OttimaSystem executa **estratégias de controle avançado (APC)** sobre plantas industriais: o engenheiro monta a lógica num **canvas de blocos** (leitura/escrita OPC-UA, MPC, script Python, simulador TFS, filtros de sinal, fuzzy, PID), o **motor** executa essa lógica ciclicamente no servidor, e o **operador** conduz o MPC por uma tela de operação com faceplates e tendência com **predição futura**. O controle regulatório permanece tipicamente nos PIDs do PLC — o OttimaSystem assume e devolve malhas de forma **bumpless** e falha sempre para o lado seguro (PLC no comando); para malhas sem PID de campo, um bloco PID interno está disponível no canvas (ADR-031).
...
- **RF-615** **Faixa do SP no SSTO** (`sp_range_pct`, % do span da CV; `null` = livre) — por `kind` da linha: **selfreg**, com SP do operador disponível, os limites da linha no LP ficam presos a `SP ± pct/100 × span` (o estático que conflite cede à banda, nível); **integrating**, vira o **ε de taxa da própria linha** (`pct/100 × span/tss`), no lugar do `economics.integrating_tolerance` do bloco — sem o campo, cai no `integrating_tolerance` do bloco (comportamento anterior). (ADR-027 §4, emenda 2026-08-17)
...
### 5.13 Bloco PID (ADR-031)
- **RF-551** Bloco `pid` com portas fixas **`pv`** (entrada, obrigatória), **`sp`** (entrada, opcional — sobrepõe o `setpoint` da config quando conectada) e **`out`** (saída), todas numéricas — ao contrário de Script/Fuzzy, o conjunto de portas não é configurável. Config na **forma ISA** (`out = Kc*[e + (1/Ti)*∫e dt + Td*de/dt]`), convertida uma única vez para a forma paralela do `simple-pid` (`Kp=Kc`, `Ki=Kc/Ti` se `Ti>0` senão `0`, `Kd=Kc*Td`): **`kc`** (ganho, qualquer sinal — negativo é ação reversa), **`ti_seconds`** (tempo integral em **s/repetição**, `>=0`, **`0` desliga a ação integral**), **`td_seconds`** (tempo derivativo em **segundos**, `>=0`, `0` desliga a ação derivativa), **`setpoint`**, **`output_min`**/**`output_max`** (nuláveis, `output_min < output_max` estrito quando ambos informados), **`auto_mode`**, **`proportional_on_measurement`**, **`differential_on_measurement`** e **`starting_output`** (dentro de `[output_min, output_max]` quando ambos informados, senão erro 422). Os parâmetros `sample_time`, `error_map` e `time_fn` do `simple-pid` **não são expostos** — o primeiro é fixado internamente, os outros dois são callables Python não serializáveis. (ADR-031)
- **RF-552** A cada varredura o bloco chama o controlador com **`dt` = Ts nominal do flow** (nunca tempo de parede medido). Cold start ⇒ saída nula e inválida. Amostra finita com `ok=False` é processada e a flag propagada (mesma regra A-6 dos filtros, ADR-026). **PV ou SP não-finito nunca chega ao controlador** — evita envenenar o termo integral permanentemente; saída não-finita, saída `None` (antes da 1ª execução válida) e exceção do `simple-pid` retêm o **último valor bom** com `ok=False`, sem relançar. `nan` **nunca** propaga com `ok=True` para a porta `out`. (ADR-031)
- **RF-552a** O save valida também os ganhos **derivados** da conversão ISA: `Ki = kc/ti_seconds` e `Kd = kc*td_seconds` precisam ser finitos (422 em pt-BR quando estouram para `inf` por overflow, ex.: `ti_seconds = 1e-320`). Campos finitos isoladamente não bastam: `Ki = inf` envenena o termo integral do `simple-pid` com `inf`/`nan` de forma permanente — o `_clamp` da lib não resgata `nan` mesmo com limites definidos —, deixando a malha presa em `ok=False` com **um único** `write_suppressed` no histórico e silêncio depois. O teto é o overflow, não um mínimo de sintonia. (ADR-031)
- **RF-553** **Reset** reconstrói o controlador (nova instância), não chama `PID.reset()` da lib — que zera o integral sem restaurar `starting_output`, aplicado só no `__init__`. **Hot-swap** preserva o termo integral enquanto a config não muda (RF-304). (ADR-031)
- **RF-554** O modal de config expõe os 10 campos com rótulos, unidades e texto de ajuda em pt-BR — o campo de `ti_seconds` mostra também o reset equivalente em repetições/segundo (`1/Ti`); limite de saída em branco significa **sem limite** naquele lado. (ADR-031)

### 5.14 Tela de operação (ADR-016)
...
### 5.16 Otimização econômica de regime permanente — SSTO (ADR-027)
