...
**Versão do documento:** 2.2 · 2026-08-14 · **Status:** aprovado para implementação (F1-F6 concluídas)
...
### 7.3 API (grupos)
`/auth` · `/users` · `/projects` (+ `/activate`, `/export`, `/import`) · `/connections` (+ certificados) · `/tags` · `/flows` (+ `/deploy`, `/stop`) · `/operate` (modos, SP, MV) · `/history` (+ `/mpc`, **`/ssto/last`**) · `/events` · `/history-retention` · `/system-settings` · `/ws` (flow.status, mpc.state, events, **opc.values filtrado por tag**).
