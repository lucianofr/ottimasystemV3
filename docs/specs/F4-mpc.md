...
> Nota (2026-08-17, extensão pós-fase sob pedido direto do operador — sem RF/fase dedicados): o item 5 acima fica emendado. Saídas do bloco ganham, além de uma por MV, **2 portas fixas de indicação de modo, sempre presentes** — `local` e `auto` — mesmo no nó recém-criado sem nenhuma variável configurada (único desvio do "100% dinâmico do config" do resto do bloco). Refletem os eixos de modo do próprio bloco (RF-621), nunca uma variável do usuário: `local` = 1.0 em LOCAL / 0.0 em REMOTO; `auto` = 1.0 em AUTO (dentro de REMOTO) / 0.0 em MAN — permanecem numéricas (decisão A-5 intacta). Saem nulas junto com as MVs sob cold start das entradas (padrão F3 §3.0); carregam o mesmo `ok` de invalidez de toda porta do bloco numa varredura ruim (decisão A-6 da F3) — nenhuma exceção só porque o valor não depende de CV/Restrição/DV. Único ponto de definição da string: `MPC_FIXED_OUTPUT_PORTS` (`ottima_core.flowgraph.mpc_config`), importado por `flowgraph.validate._output_handles`, `blocks/mpc.py` (`output_ports`/`_compute_outputs`) e, no frontend, `graph.ts` (`PORTA_MPC_LOCAL`/`PORTA_MPC_AUTO`, consumidos por `handlesSaida`/`NoMpc`). Como toda porta do MPC (decisão A-5), `local`/`auto` são numéricas — ligação direta a um `opc_write` de tag BOOL (comum para indicação de modo no PLC) reprova em 422 (`_check_port_types`: só o Script é bivalente); a ponte para um destino BOOL precisa de um bloco Script no meio, ou o destino usa tag INT/float.
...
| stop do flow em REMOTO | devolve (`mode_cmd = auto`) antes de encerrar a task | em `comm_failure` não há como escrever — o watchdog do lado do PLC devolve (ADR-009); documentado |
   ...
   Pré-condição de REMOTO: flow rodando e todas as entradas aquecidas (flow vivo ⟹ sem `comm_failure` pendente nas conexões referenciadas — §2.2-8). Boot/deploy ⇒ sempre LOCAL (RNF-03; decisão A-4). **SP faz PV-tracking fora de AUTO e congela ao entrar** (decisão A-4); modos e MV manual não persistem no banco. **Exceção do SP (emenda A-4, 2026-08-20):** o SP escrito pelo operador em AUTO persiste em `mpc_setpoints` e volta como semente de `reset()` no deploy/redeploy/restart (clampado aos `sp_limits` vigentes; CV com `track_sp` volta a rastrear o PV na primeira varredura quente fora de AUTO, como sempre).
...
6. **Invalidez (simetria F3 §3.0/decisão A-6 da F3):** entrada CV/Restrição com `ok=false` na fronteira ⇒ pula o solve, saídas mantêm valor com `ok=false` (OPC-Write a jusante suprime), escritas internas do `pid` suprimidas, `mpc_input_invalid` warning (dedupe). **DV fica FORA do gate (ADR-038):** amostra ruim de DV congela internamente — o bloco segue resolvendo com o último valor bom da DV (feedforward parado não impacta o algoritmo); ação default fixa, sem fail action nem campo de config novo. Cold start (`v=None`) ⇒ saídas nulas (§3.0 F3) e REMOTO bloqueado.
...
| A-4 | Ciclo de vida de modos/SP/MV manual | **Volátil + tracking**: deploy ⇒ LOCAL; REMOTO entra em MAN; SP faz PV-tracking fora de AUTO e congela ao entrar; modos/MV manual não persistem (RNF-03). **Emenda 2026-08-20 (orch-change-feature):** o NÚMERO do SP persiste em `mpc_setpoints` (PK `(flow_id, block_id, var_id)`, valor já clampado, FK `flows` CASCADE) e volta como semente de `reset()` — restart/redeploy/stop-start não zeram mais o SP do operador |
