...
| Item | Governança |
|---|---|
| opc-worker completo: sessões asyncua (≤5), reconciliação de config (banco + dica via `events`), polling cíclico com heartbeat de valor, reconexão com backoff, semântica de desconexão (quality=bad) | RF-201/204 · ADR-002/006/032 · §2 |
...
services/opc-worker/src/ottima_opc_worker/
  ...
  polling.py       # leitura cíclica em lote (tags R) → publica opc.values.<conn_id>
...
4. **Mapeamento tag→node_id:** o conjunto com série é decidido uma única vez na criação do poller, e a lista de tags e a de nodes nascem **na mesma iteração** — o índice é a única ligação entre a resposta do servidor e a tag, então elas são emparelhadas com `zip(..., strict=True)` (desalinhar publicaria valor sob o `tag_id` errado; resposta curta do servidor vira exceção em vez de truncar em silêncio). Entram: **toda** tag `direction='r'`; e tag `direction='w'` **cujo node declare o bit CurrentRead no AccessLevel** — o valor de uma tag `w` é o comando em vigor no servidor, grandeza distinta do readback de posição real (RF-604) e dado de processo por direito próprio; `direction` governa quem o sistema pode ESCREVER, não o que ele pode observar. Leitura do AccessLevel é **otimista** (servidor que não expõe o atributo conta como legível) e feita num único round trip para todos os candidatos. Comando **write-only** é caso legítimo: fica fora do ciclo, sem série, sem contador e sem evento — publicar bad mostraria "ruim 0" onde o honesto é "sem dado". Node inexistente/inválido em tag `r` não derruba a conexão: o `DataValue` volta com StatusCode Bad, a tag vira quality=2 **a cada varredura** (a série continua viva como ruim) e o evento warning `tag_subscribe_error` é emitido **uma vez por tag** — `read_errors` conta TAGS em falha, não ocorrências, senão cresceria sem limite no polling. Nome do `kind` mantido por ser contrato do canal `events` (§7.3), ainda que "subscribe" já não descreva o mecanismo. **[NOVA — implementação]**
5. **Polling:** 1 task por conexão; a cada ciclo, `client.read_attributes(nodes, ua.AttributeIds.Value)` devolve `list[ua.DataValue]` para **todas as tags com série** da conexão (o conjunto do §2.2-4) num único round trip. Período configurável **por conexão** (`opc_connections.polling_period_ms`, default **1000 ms**, faixa **100–60000 ms**, `CheckConstraint`); ele fica **fora da `session_key`** — mudar a varredura retima o poller sem recriar a sessão asyncua (§2.2-1). Sleep entre ciclos compensado pelo tempo gasto na leitura, sem acúmulo de drift; timeout de I/O `max(10, 3*período)`. StatusCode OPC → quality `0/1/2` (Good/Uncertain/Bad — spec F1 §3.4-4) por tag; publica `OpcValue` (payload §7.1 verbatim, tipado em `ottima_core.bus`) para **todas** as tags a cada ciclo e atualiza o snapshot — não é mais report-by-exception. Exceção na leitura leva a conexão a `failed` com `reason="session_lost"` (§3.6); o `stop()` do poller guarda contra reentrância, porque essa cadeia de falha nasce dentro da própria task do ciclo. **[NOVA — implementação]** (substitui subscription/monitored items da decisão original de F2, revertida por ADR-032)
6. **Heartbeat de valor:** mecanismo inalterado — a cada 10 s republica cada tag sem publicação há ≥10 s (mesmo valor/qualidade, ts novo). Com polling, toda tag já publica a cada ciclo (§2.2-5); enquanto o `polling_period_ms` da conexão for **≤ 10 s** o heartbeat é efetivamente no-op (nunca encontra uma tag "sem publicação há ≥10 s"); só volta a preencher lacuna em conexões com período **> 10 s**. Conexão em falha ⇒ heartbeat segue com `quality=2` para todas as tags dela (valor = último conhecido; sem último ⇒ `0.0`, irrelevante sob bad). Na detecção da falha, rajada imediata de quality=bad para todas as tags da conexão. Na reconexão, o próximo ciclo de polling restaura os valores. **[NOVA — implementação]** (revisão ADR-032; semântica de falha inalterada)
7. **Timestamps/serialização:** `ts` = **instante da leitura** (`now()` UTC), não o SourceTimestamp do servidor. A regra mudou com o ADR-032: sob subscription cada notificação ERA uma mudança, e o SourceTimestamp era o eixo de tempo certo; sob polling cada ciclo é uma **observação**, e o SourceTimestamp de um node estático não avança nunca — usá-lo empilharia todas as amostras no mesmo `x` do trend, inflaria `n_samples` no CAgg `samples_1m` (`samples` não tem unique em `(tag_id, ts)`, então nada barraria a duplicata) e faria um node parado parecer eternamente velho para qualquer checagem de frescor. É o mesmo carimbo que a republicação do heartbeat sempre usou (§2.2-6). ISO-8601 UTC no JSON; `value` float (bool→0/1, int→float — `samples.value` é DOUBLE PRECISION, spec F1 §3.2). **[NOVA — implementação]** (revisão ADR-032)
8. **`/health` (RNF-07):** `{status, service, version, connections:{<id>:{name, state, watchdog_alive, session_up_since, last_publish_ts, tags_polled, read_errors, write_errors}}}`. **`status` reflete só as dependências do serviço (Redis/banco)** — conexão OPC caída é condição operacional (alarme), **não** unhealth do serviço: o healthcheck do compose não pode reiniciar o worker porque um PLC está desligado. **[NOVA — implementação]** (renomeado `tags_subscribed`→`tags_polled`, `monitored_errors`→`read_errors` por ADR-032)

---

## 3. Watchdog e política de falha

1. **Task por conexão** (só quando o par de node_ids está configurado), cadência = `watchdog_period_ms` (500–5000 ms, default 1500 — DDL F1). Ciclo: `read` **explícito** do bit de leitura → se mudou, registra transição → escreve `NOT(valor lido)` no bit de escrita. Leitura dedicada, independente do ciclo de polling de tags (§2.2-5): congelamento medido deterministicamente. (ADR-009, RF-206)
...
- **opc-worker contra servidor asyncua in-process** (CLAUDE.md §Testes): polling→`OpcValue` · heartbeat 10 s · transição de quality · escrita + auditoria · gate (sem watchdog / em falha / reabertura pós 1ª alternância) · congelamento >10 s · reconexão com backoff · reconciliação (tag set ⇒ recria poller; conexão ⇒ recria sessão) · `cert_missing`/`cert_mismatch` · 3 modos de segurança (certs de fixture).
...
|---|---|
| **Leituras de servidor real chegam ao trend** | cadeia §2 (polling) → §6 (recorder) → §8 (history) → §9 (trend); E2E-F2-01/02 + B-04 |
...
| 3 | Política de publicação | **Revertido por ADR-032:** polling cíclico por conexão (`opc_connections.polling_period_ms`, default 1000 ms, faixa 100–60000 ms), 1 round trip por ciclo para todas as tags **com série** (§2.2-4: toda `r`, mais a `w` cujo node declare `CurrentRead`); heartbeat de valor a cada 10 s mantido (vira no-op com período ≤10 s) |
