...
> Nota (spec F5 §1.2): o registro de valores de tag (`opc.values.<conn_id>` no `/ws`) é reapontado — fica F6 ou nunca, só com consumidor real; eventos (faixa anunciadora) seguem F5.
...
11. **Partição por processo (ADR-004, 2026-08-15):** `OTTIMA_FLOW_PARTITIONS` (default **1**, o comportamento acima sem alteração alguma). Acima de 1, o processo do compose vira **pai** — não executa flow, dá `spawn` em N filhos e agrega o `/health` na mesma porta 8002, no MESMO formato (`flows` é a união das fatias, `script_pool` é a soma; a chave nova `partitions` é aditiva). Posse por `flow_id % N`, aplicada em `Supervisor.handle_command`: `flow.commands` é canal único, então toda partição recebe todo comando e quem não é dono descarta em silêncio — sem log por comando, que com N partições viraria ruído. Basta um ponto porque a decisão #1 acima (boot parado) garante que nenhum outro caminho sobe flow. **Isola o tempo de varredura entre partições, não dentro de uma** — flows da mesma partição continuam dividindo o event loop (`tests/test_isolamento_temporal.py`, `xfail`).
...
- **Arestas vestem a qualidade do sinal de origem (RF-305, emenda 2026-08-18):** com `ports` ao vivo, a aresta lê `ports[source][sourceHandle].ok` — `ok=true` ⇒ espessura viva 3.5 px na cor normal (edição = 1.5 px); `ok=false` ⇒ magenta (`--edge-bad`) em 3.5 px + marcador **X 5 unidades à frente do handle de saída e 8 acima do traço** (decorativo, `aria-hidden` — o canal textual de invalidez continua no nó, Regra do Canal Redundante). Sem dado ao vivo (modo edição ou antes da 1ª varredura) ⇒ estilo de edição; flow parado **preserva a última qualidade** (`mesclarPorts`, mesmo critério dos valores nos nós). Aresta BAD selecionada permanece magenta+X (especificidade sobre o `.selected` do base.css).
