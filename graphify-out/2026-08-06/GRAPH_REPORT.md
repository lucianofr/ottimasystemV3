...
## Corpus Check
- 350 files · ~268,313 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 5002 nodes · 12245 edges · 280 communities (202 shown, 78 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 807 edges (avg confidence: 0.7)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `fa4674a2`
...
## Communities (280 total, 78 thin omitted)
...
Cohesion: 0.06
Nodes (79): _BrokenPubSub, CancellingRuntime, ClosingRedis, Contador, contar_mensagens_vistas(), contar_passadas(), create_connection(), create_project() (+71 more)
...
Cohesion: 0.08
Nodes (60): MpcConfig, MvVar, Config do bloco MPC — vive inteiro no `graph_json` (spec §2.1, decisão A-8/A-9)., Variável manipulada. Sem `pid` ⇒ MV "direta" (spec §2.1-3)., FlowEdge, FlowGraph, FlowNode, _check_cycles() (+52 more)
...
Cohesion: 0.09
Nodes (19): FlowTask, AsyncSession, Block, Flow, Mantém as `FlowTask` alinhadas com os comandos e com o banco., Tasks vivas por `flow_id` — inclui as paradas e as em falha, como o `/health`., Delega a `MpcOrchestrator.mpc_health` (spec F4 §4.10, plano F4b tarefa 2.3/5.0)., Delega a `MpcOrchestrator.script_pool_stats` (débito 5, spec F4 §4.10/§8). (+11 more)
...
Cohesion: 0.14
Nodes (43): create_connection(), create_tag(), delete_flow(), disabled_element(), edge(), graph(), mpc_graph_valido(), node() (+35 more)
...
Cohesion: 0.07
Nodes (26): ABC, Block, has_cold_input(), null_outputs(), PortSample, Protocolo comum dos blocos executáveis e as duas regras de base (spec F3 §3.0).…, Valor de uma porta numa varredura. `v is None` é cold start (nunca houve valor…, Bloco de um flow. `block_id` é o id do nó React Flow (chave do hot-swap,… (+18 more)
...
Cohesion: 0.12
Nodes (30): DM, DMStruct, BuiltMpc, Controller do-mpc pronto + metadados de índice para o worker (spec F4 §3,…, Init bumpless — rotina única de armar/re-armar (spec F4 §3.6; TDD estrito).…, `MpcHost` — dono, no processo pai, do processo filho do worker MPC (spec F4…, Montagem do-mpc do bloco MPC (spec F4 §3) + worker do processo filho (spec F4…, _apply_tvp() (+22 more)
...
Cohesion: 0.10
Nodes (15): _flow_id_of(), flow_status_ws(), FlowStatusHub, _mpc_id_of(), Assinatura única de `flow.status.*` e `mpc.state.*` roteando aos sockets…, Assina os dois padrões e sobe as tasks de leitura; retorna já. Idempotente. O…, Para os laços, encerra as inscrições e fecha os sockets restantes. Nunca…, Roteia uma publicação para quem pediu aquele flow/bloco. Não aguarda nenhum… (+7 more)
...
Cohesion: 0.10
Nodes (35): Montagem do-mpc do bloco MPC (spec F4 §3.2-3.5; TDD estrito). `build_mpc` monta…, _delay_samples(), discretize_iopdt(), discretize_sopdt(), PairSS, Discretização ZOH-exata por par SOPDT/IOPDT no `Ts_mpc` (spec F4 §3.1; TDD…, IOPDT: integrador retangular `acc += Ki*Ts*u` — idêntico ao `_Iopdt` do TFS, 1…, Modelo discreto de um par da matriz MPC, no `Ts_mpc`, sem o atraso. `a`: matriz… (+27 more)
...
Cohesion: 0.07
Nodes (22): _LocalRemote, MpcModes, MpcPrediction, MpcStatus, MpcVarState, BaseModel, Estado publicado de uma variável do MPC (spec F4 §5.1) — `sp` só existe em CV…, _clamp() (+14 more)
...
Cohesion: 0.09
Nodes (35): BaseException, AppCertPaths, Caminhos dos três arquivos do certificado de aplicação., CertMismatchError, CertMissingError, _configure_channel(), configure_client(), _configure_identity() (+27 more)
...
Cohesion: 0.15
Nodes (26): NodeConfig, Modelo tipado do `graph_json` de um flow + validação compartilhada (RF-302/307,…, GraphParseError, IopdtParams, _is_int(), _is_number(), MpcRawConfig, _parse_config() (+18 more)
...
Cohesion: 0.07
Nodes (34): Snapshot em memória do worker inteiro; a fonte do /health (spec §2.2-8)., Tag configurada de uma conexão., TagConfig, WorkerState, _cancel(), _is_hint(), load_active_configuration(), _log_teardown_results() (+26 more)
...
### Community 76 - "FlowCommand"
Cohesion: 0.17
Nodes (26): BlockId, FlowCommand, _cv_do_bloco(), ModeCommand, _mpc_config(), _mv_do_bloco(), MvCommand, _publicar_comando() (+18 more)
...
### Community 105 - "SolveResult"
Cohesion: 0.09
Nodes (16): MpcState, Publicado em `mpc.state.<flow_id>.<block_id>` a cada execução (spec F4 §5.1,…, Resposta de solve — picklável. Ordem das linhas de…, SolveResult, Último valor conhecido de uma tag, como veio do barramento. `value` continua…, Último valor da tag, ou `None` se ela nunca publicou. `None` é o cold start da…, Grava o último valor da tag; payload ruim é descartado e o laço segue., TagValue (+8 more)
...
### Community 114 - "bus.py"
Cohesion: 0.10
Nodes (26): Contratos do barramento Redis pub/sub — payloads verbatim do PRD §7.1…, Bloco MPC: cadência, modos e write-back do runtime (spec F4…, Bloco OPC-Read: uma tag do espelho do barramento vira uma porta de saída…, build_definition(), _instantiate(), _instantiate_mpc(), _make_write_opc(), Any (+18 more)
...
### Community 128 - "routers/history.py"
Cohesion: 0.16
...
### Community 149 - "ottima_flow_runtime/supervisor.py"
Cohesion: 0.12
Nodes (19): Laço resiliente de assinatura ao barramento pub/sub do Redis (canal fixo ou…, project_tags(), AsyncSession, Consulta de tags do projeto: compartilhada entre API e flow-runtime., Tags visíveis ao flow: as do projeto dele, via conexão (o `graph_json` não tem…, Duas conexões, dois projetos: `project_tags` não pode vazar tag de outro…, test_so_traz_tags_das_conexoes_do_projeto(), flow_origin() (+11 more)
...
Cohesion: 0.12
Nodes (17): FlowStatus, OpcValue, async_sessionmaker, AsyncSession, fixture, Fixtures compartilhadas dos testes do flow-runtime. As fixtures de…, Factory commitada de verdade: o supervisor não vê transação aberta de teste., session_factory() (+9 more)

### Community 165 - "ottima_recorder/main.py"
Cohesion: 0.13
Nodes (14): check_redis(), health(), _heartbeat_loop(), lifespan(), FastAPI, Serviço recorder: /health + heartbeat de Redis (F1) e o pipeline de gravação…, Faz ping no Redis e registra o resultado em app.state.redis_ok., Repete o ping no Redis a cada HEARTBEAT_INTERVAL_S segundos. (+6 more)
...
Cohesion: 0.17
Nodes (15): LogRecord, JsonFormatter, Serializa cada registro como uma linha JSON com timestamp UTC., Substitui os handlers do logger raiz por um único handler JSON em stdout., setup_logging(), check_database(), check_redis(), _heartbeat_loop() (+7 more)
...
### Community 273 - "PatternListener"
Cohesion: 0.22
Nodes (5): PatternListener, Redis, Assinante resiliente de um padrão glob (`PSUBSCRIBE`); `handler` recebe…, Redis, Redis

### Community 274 - ".stop"
Cohesion: 0.20
Nodes (8): _cancel(), _log_teardown_results(), Any, Task, Derruba poll, consumidor, todas as tasks e o pool — nesta ordem. Idempotente.…, Para a task e tira o flow do mapa e do `/health`. A entrada sai do mapa mesmo…, Registra o que o gather engoliu: desmonte silencioso esconde flow órfão…, Cancela e aguarda a task; erro dela não pode impedir o resto do desmonte.

### Community 275 - ".__init__"
Cohesion: 0.25
...
### Community 276 - "PairInit"
Cohesion: 0.29
Nodes (7): ndarray, PairInit, Metadados de um par HABILITADO para `init_bumpless` (spec F4 §3.6, tarefa 2.3).…, _pair_input_value(), Entrada vigente da coluna do par — MV ou DV, mesmo valor usado no `x_ss` e nos…, `x = x_ss(u, d)` de um par autorregulável: resolve `(I−A)·x = B·u_eff` (spec…, _steady_state()

## Knowledge Gaps
- **340 isolated node(s):** `1.1 Dentro da F5`, `1.2 Fora da F5 — com destino registrado`, `1.3 Emendas a documentos anteriores (consolidação; F5R-26)`, `2.1 Emenda PRD §7.1 → v1.3: `ts` e `prediction.ts` no `mpc.state` (decisão A-2; F5R-01)`, `2.2 Hypertable `mpc_samples` (decisão A-1; migration `0003_mpc_samples`, SQL cru, três passos como a 0002)` (+335 more)
...
- **Why does `OpcWrite` connect `OpcWrite` to `OpcSim`, `SolveResult`, `MpcOrchestrator`, `Ambiente`, `TagConfig`, `bus.py`, `MpcBlock`, `FlowStatus`, `test_writes.py`, `collect`, `Any`, `PortSample`?**
  _High betweenness centrality (0.025) - this node is a cross-community bridge._
- **Why does `await_until()` connect `await_until` to `opc-worker/tests/test_supervisor.py`, `FakeClock`, `MpcHost`, `processo_vivo`, `test_watchdog.py`, `collect`, `runtime_test_helpers.py`, `opc-worker/tests/test_security.py`, `Collector`, `harness_factory`, `test_failure.py`, `test_backpressure.py`, `ChannelListener`, `test_connection.py`, `test_subscriptions.py`, `RecorderPipeline`, `test_heartbeat.py`, `test_pipeline.py`, `test_server.py`, `test_busy_loop_nao_trava_o_event_loop`, `test_dupla_cancelacao_durante_replace_nao_encolhe_o_pool`, `test_stats_conta_respawns_e_reflete_ocupacao`, `test_writes.py`, `test_script.py`?**
  _High betweenness centrality (0.021) - this node is a cross-community bridge._
- **Why does `ConnectionConfig` connect `ConnectionConfig` to `logging.py`, `ConnectionRuntime`, `Supervisor`, `WatchdogTask`, `ValueSubscription`, `ValueHeartbeat`?**
...
- **What connects `1.1 Dentro da F5`, `1.2 Fora da F5 — com destino registrado`, `1.3 Emendas a documentos anteriores (consolidação; F5R-26)` to the rest of the system?**
