...
## Knowledge Gaps
- **521 isolated node(s):** `Contexto`, `1. Camada e localização`, `2. Formulação`, `3. G e Gd reutilizam o modelo existente — não há segundo modelo de ganho`, `4. Linhas integradoras (`kind="integrating"`)` (+516 more)
...
- **Why does `publish_event()` connect `publish_event` to `opc-worker/tests/test_supervisor.py`, `ConnectionRuntime`, `ResumeOrchestrator`, `Collector`, `FlowTask`, `PortSample`, `test_kind_fora_de_hint_kinds_nao_dispara_reconcile`, `WriteConsumer`, `harness_factory`, `test_backpressure.py`, `routers/flows.py`, `RecorderPipeline`, `test_pipeline.py`, `subscriptions.py`, `routers/connections.py`, `create_flow`, `routers/projects.py`, `User`, `bus.py`?**
