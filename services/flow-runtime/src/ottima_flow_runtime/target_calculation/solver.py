"""Backends de solver do SSTO — interface plugável (ADR-026 §7).
...
- `OSQPBackend` — QP do detuning anti-flipping (`solver_qp.py`, ADR-026 §8).
...
(`ssto.py`) carimba quando chegou a uma solução desistindo de linhas por rank (ADR-026 §6).
...
class QuadraticTerm:
    """Termo `ρ‖x − x_ref‖²` do detuning anti-flipping (ADR-026 §8).
...
@dataclass(frozen=True, slots=True, eq=False)
class SolverResult:
    """Resposta de um backend — tudo que a auditoria (ADR-026 §11) precisa registrar.
...
class SolverBackend(Protocol):
    """Contrato de um backend (ADR-026 §7)."""
...
class HiGHSBackend:
    """LP por `scipy.optimize.linprog(method="highs")` — backend default (ADR-026 §7)."""
...
@dataclass(frozen=True, slots=True)
class GurobiBackend:
    """Stub declarado (ADR-026 §7): a interface suporta o plugin comercial, a implementação
    não é desta fase."""
    ...
    def solve(
        ...
        raise NotImplementedError("GurobiBackend ainda não implementado (ADR-026 §7)")
