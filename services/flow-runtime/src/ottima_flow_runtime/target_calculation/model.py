"""Modelo de regime permanente do SSTO: `G`, `Gd` e a base de predição (ADR-026 §3/§4).
...
- **`integrating` (IOPDT)** — não tem ganho estático finito (`a = 1`, `(I − a)` singular):
  ...
  entra no LP como condição de **taxa nula em regime** (ADR-026 §4), nunca como nível.
...
class SteadyStateModel:
    """`G`/`Gd` + metadados de ordem (ADR-026 §3).
...
def build_steady_state_model(config: MpcConfig, ts_mpc: float) -> SteadyStateModel:
    """Monta `G`/`Gd` a partir da matriz `models` do config, via `PairSS` (ADR-026 §3)."""
