"""Integração SSTO ↔ worker do MPC (ADR-026 §1/§10).
...
def test_alvo_e_limitado_pelos_sp_limits_da_cv():
    """`sp_limits` é a faixa admissível do alvo (ADR-026 §5) — o LP não pode entregar SP
...
def test_cv_integradora_mantem_o_sp_do_operador():
    """Linha integradora não tem alvo de NÍVEL (ADR-026 §4): o LP só decide a taxa, então o
...
def test_dv_anterior_e_lembrada_entre_execucoes():
    """O `ΔDV` do ciclo é medido contra a DV da execução ANTERIOR (ADR-026 §2) — quem
...
def test_worker_module_nao_importa_do_mpc_no_target_calculation():
    """Separação física das camadas (ADR-026 §1): o pacote do SSTO não conhece do-mpc."""
