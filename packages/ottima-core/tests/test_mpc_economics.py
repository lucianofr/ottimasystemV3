"""Mesa de casos do bloco `economics` do `MpcConfig` (ADR-026 §9, RF-901/902).
...
def test_cv_sem_priority_recebe_rank_default():
    """`CvVar.priority` é campo novo (ADR-026 §5): default neutro, todas as CVs iguais."""
    config = MpcConfig.model_validate(base_config())
...
# ---------------------------------------------------------------------------------------
# config_hash (ADR-026 §9): identidade da versão de custos/limites/ranks
# ---------------------------------------------------------------------------------------
