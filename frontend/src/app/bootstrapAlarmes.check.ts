 ...
 * evento ao vivo — o REST não tem TTL. Cobre: teto de chamadas do grupo 1 (par), uma
 * severidade por chamada + janela de 2h do grupo 2 (demais famílias), cache de 60s, a
 * mescla dos dois grupos, e a integração com `resolverAlarmes` (último evento = abertura
 * ⇒ ativa; par já fechado ⇒ inativa).
 */
...
test("grupo 1: uma chamada origin por id, teto de 10 flows e 5 conexões, limit 20", async () => {
  ...
  await bootstrapAlarmes({ flowIds, connectionIds }, AGORA, ambiente);

  const deFlow = chamadas.filter((c) => c.includes("origin=flow:"));
...
test("grupo 2: a janela é recalculada a partir de `agora`, nunca fixa", async () => {
  ...
  await bootstrapAlarmes({ flowIds: [], connectionIds: [] }, outroAgora, ambiente);
...
test("cache: mesmo escopo dentro de 60s reaproveita, expira depois e refaz as chamadas", async () => {
  ...
  const escopo = { flowIds: [1], connectionIds: [] };
...
test("cache: escopo diferente não reaproveita, mesmo dentro da janela de 60s", async () => {
  ...
  await cache.obter({ flowIds: [1], connectionIds: [] }, AGORA, ambiente);
  const apos1a = chamadas.length;

  await cache.obter({ flowIds: [2], connectionIds: [] }, AGORA, ambiente);
  expect(chamadas.length).toBeGreaterThan(apos1a);
});
