 ...
 * as 3 entradas na ordem opc_worker/flow_runtime/recorder e traduz `up` em rótulo + estado
 * textual (Regra do Canal Redundante, DESIGN.md §Colors), independente do que mais o
...
test("sem resposta ainda: as 3 lâmpadas ficam indisponíveis, nunca somem", () => {
...
test("todos up: as 3 ficam ativas, na ordem opc_worker/flow_runtime/recorder", () => {
  const lampadas = derivarLampadas(saude());
  expect(lampadas.map((l) => l.id)).toEqual(["opc_worker", "flow_runtime", "recorder"]);
...
test("um worker down: só a lâmpada dele muda, com rótulo textual junto (não só cor)", () => {
  ...
  expect(lampadas.filter((l) => l.ativo).map((l) => l.id)).toEqual(["opc_worker", "flow_runtime"]);
});
